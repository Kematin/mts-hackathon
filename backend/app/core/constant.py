...
=== ПРАВИЛА ===
1. Используется Lua 5.4 и только его. только Lua
...
10. Пиши МИНИМАЛЬНЫЙ код. Без лишних вспомогательных функций. Весь ответ должен уместиться в 400 токенов.
11. Никогда не используй os.time(), os.date(), io.*, os.* — этих функций нет на платформе.
12. Используй ipairs() для массивов, pairs() для таблиц.
...
Всегда используй return. Никогда не используй os.time(), os.date(), io.*, os.*.
Отвечай ТОЛЬКО валидным JSON. Никаких пояснений, никакого markdown.
"""

CLARIFIER_SYSTEM_PROMPT = """Ты — ассистент который решает нужно ли уточнение перед генерацией Lua-кода.

Уточнение НУЖНО только если запрос АБСОЛЮТНО невозможно выполнить без дополнительной информации:
- Не указано ВООБЩЕ никакое имя переменной и нет контекста wf.vars

Уточнение НЕ НУЖНО если:
- Указано любое имя переменной или объекта
- Передан контекст wf.vars
- Запрос содержит конкретное действие (увеличь, получи, конвертируй, отфильтруй)
- Запрос понятен хотя бы частично — генерируй код с разумными допущениями

ВАЖНО: Если в тексте есть раздел "Контекст:" — уточнение НЕ НУЖНО никогда, сразу возвращай false.

Если сомневаешься — НЕ спрашивай, генерируй.

Отвечай ТОЛЬКО валидным JSON:
{"need_clarification": true, "question": "Короткий вопрос на русском"}
или
